            ...
            <MenuItems :class="[
                ...
                <div class="py-1">
                    ...
                    <a @click="goToSelectAll()"
                        ...
                        <CheckIcon :class="[active ? 'text-blue-500' : '', 'mr-3 size-5']" aria-hidden="true" />
                        Select all
                    </a>
                    </MenuItem>
                </div>
                <div v-if="props.select.length > 0" class="py-1">
                    <MenuItem v-slot="{ active }">
                    <a @click="goToSignCertificate()"
                        :class="[active ? 'bg-gray-100 text-blue-600 outline-none' : 'text-gray-700', 'group flex items-center px-4 py-2 text-sm cursor-pointer']">
                        <PlusIcon :class="[active ? 'text-blue-500' : '', 'mr-3 size-5']" aria-hidden="true" />
                        Sign certification
                    </a>
                    </MenuItem>
                </div>
...
const emit = defineEmits(['selectAll', 'signCertificate'])
...
function goToSignCertificate() {
    emit('signCertificate')
}
